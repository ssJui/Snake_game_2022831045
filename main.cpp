...
bool init() {
    ...
    gWindow = SDL_CreateWindow("Snake Game by JUI", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
    if (gWindow == nullptr) {
    ...
    SDL_Surface* surface = IMG_Load("background.jpeg");
    if (surface == nullptr) {
    ...
    // Load logo image
    surface = IMG_Load("Loopie-Photoroom.png");
    if (surface == nullptr) {
        std::cerr << "Failed to load logo image! SDL_Error: " << SDL_GetError() << std::endl;
        return false;
    }
    gLogoTexture = SDL_CreateTextureFromSurface(gRenderer, surface);
    SDL_FreeSurface(surface);
    if (gLogoTexture == nullptr) {
        std::cerr << "Failed to create texture from logo image! SDL_Error: " << SDL_GetError() << std::endl;
        return false;
    }
    ...
    // Load font
    gFont = TTF_OpenFont("fonts/Kanit-SemiBold.ttf", 28);
    if (gFont == nullptr) {
...
void renderWelcomeScreen() {
    SDL_SetRenderDrawColor(gRenderer, 255, 255, 255, 255);  // Set the background color to white
    ...
    int logoWidth = 300;  
    int logoHeight = 300; 
    SDL_Rect logoRect = { SCREEN_WIDTH / 2 - logoWidth / 2, SCREEN_HEIGHT / 2 - 200, logoWidth, logoHeight }; 
    SDL_RenderCopy(gRenderer, gLogoTexture, nullptr, &logoRect);

    // Render Start button
    SDL_Rect startButtonRect = { SCREEN_WIDTH / 2 - 130, SCREEN_HEIGHT / 2 + 100, 100, 50 }; // Left button
    SDL_SetRenderDrawColor(gRenderer, 0, 255, 0, 255); // Green color
    SDL_RenderFillRect(gRenderer, &startButtonRect);
    renderText("Start", SCREEN_WIDTH / 2 - 120, SCREEN_HEIGHT / 2 + 110);

    // Render Exit button
    SDL_Rect exitButtonRect = { SCREEN_WIDTH / 2 + 30, SCREEN_HEIGHT / 2 + 100, 100, 50 }; // Right button
    SDL_SetRenderDrawColor(gRenderer, 255, 0, 0, 255); // Red color
    SDL_RenderFillRect(gRenderer, &exitButtonRect);
    renderText("Exit", SCREEN_WIDTH / 2 + 50, SCREEN_HEIGHT / 2 + 110);
        ...
        while (SDL_PollEvent(&e) != 0) {
            if (e.type == SDL_QUIT) {
                ...
                if (gameState == GameState::WELCOME_SCREEN) {
                    ...
                    if (mouseX >= SCREEN_WIDTH / 2 - 130 && mouseX <= SCREEN_WIDTH / 2 - 30 &&
                    ...
                    else if (mouseX >= SCREEN_WIDTH / 2 + 30 && mouseX <= SCREEN_WIDTH / 2 + 130 &&
                             mouseY >= SCREEN_HEIGHT / 2 + 100 && mouseY <= SCREEN_HEIGHT / 2 + 150) {
                        quit = true;
                    }
                }
